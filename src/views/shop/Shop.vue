      ...
      <div class="plans-wrapper">
        ...
        <div class="plan-card" v-else v-for="plan in filteredPlans" :key="plan.id">
          ...
          <div class="card-body">
            ...
            <!-- 周期折扣 / 一次性信息 -->
            <div class="discount-calculation" v-if="SHOP_CONFIG.enableDiscountCalculation && calculateDiscount(plan).showDiscount && !isOnetimeOnly(plan)">
              <div class="discount-info">
                ...
                <span class="saving-amount">&nbsp;{{ calculateDiscount(plan).discountPercentage }}%</span>
                <span class="saving-text">，{{ $t('shop.plan.discount.savings') }} </span>
                <span class="discount-value">&nbsp;{{ currencySymbol }}{{ calculateDiscount(plan).savingsAmount }}</span>
              </div>
            </div>
...
export default {
  ...
  setup() {
    ...
    const calcMonthlyAverage = (type, cents) => {
      const m = periodMonthsMap[type];
      if (!m || m === 1) return formatPrice(cents);
      return (cents / m / 100).toFixed(2);
    };
    ...
    const getPlanMainPrice = (plan) => {
      ...
      if (type === 'onetime_price') return formatPrice(v);
      return calcMonthlyAverage(type, v);
    };

    const getMonthlyAveragePrice = (plan, type) => {
      const v = plan[type];
      if (v === null || v === undefined) return '--';
      if (type === 'onetime_price') return formatPrice(v);
      return calcMonthlyAverage(type, v);
    ...
    const getOnetimePricePerGB = (plan) => {
      if (plan.onetime_price == null) return '--';
      const totalBytes = plan.transfer_enable || plan.transferEnable || plan.total_transfer || 0;
      if (!totalBytes) return '--';
      if (totalBytes <= 0) return '--';
      const price = plan.onetime_price / 100;
      return (price / totalBytes).toFixed(2);
    };
    ...
    return {
      ...
      getMonthlyAveragePrice,

      getOnetimePricePerGB,
    ...
    .discount-calculation {
      ...
      .discount-info {
        ...
        .discount-value {
          ...
          animation: discount-scale 2.4s ease-in-out infinite;
